...
#include "strappy_fontawesome_icons.inc"
    ...
    "function renderAPIToolRows(){var rows=apiToolRows();var calls={};var dbNames={};",
    ...
    "((id!==''&&calls[id])?calls[id].args:'');title=toolDisplayTitle(name,args,dbNames);",
    ...
    "var dbNames;var i,row,raw,events,c,target;clearToolBoxes();dbNames=collectDatabaseNames(rows);",
    "for(i=0;i<rows.length;i++){row=rows[i];raw=toolRowRaw(row);if(raw==='')continue;",
    ...
    "else{c=pending[target].length?pending[target].shift():null;if(c){c.output=raw;c.error=hasClass(row,'state-error')||toolOutputHasError(raw);}",
    "else{grouped[target][grouped[target].length]={name:'Tool Result',id:'',args:'',output:raw,outputOnly:true,error:hasClass(row,'state-error')||toolOutputHasError(raw)};}}}",
    ...
    "function appendMessage(html){clearTimelineError();var m=byId('messages');var d,n,id,key,added=0;if(!m)return;",
    ...
    "m.appendChild(n);added=1;}if(!added)return;",
    "renderAfterMutation(m);scrollBottomAnimated();}",
    ...
    "function insertMessageBefore(id,html){clearTimelineError();var before=byId(id);var m=byId('messages');var n,added=0;",
    "if(!m){return;}if(!before){appendMessage(html);return;}var d=nodesFromHTML(html);",
    "while(d.firstChild){n=d.firstChild;m.insertBefore(n,before);added=1;}",
    "if(!added)return;renderAfterMutation(m);scrollBottomAnimated();}",
    ...
    "b._strappyRawText+=t;renderAfterMutation(document);}",
