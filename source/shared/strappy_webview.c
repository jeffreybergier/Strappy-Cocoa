...
static int strappy_webview_append_scripts(strappy_webview_buffer *buffer)
{
  static const char * const chunks[] = {
    ...
    "renderMetadata(document);scrollBottom();}",
    ...
    "var l=byId('load-more');if(l&&!hasMore)l.parentNode.removeChild(l);renderMetadata(m);}",
    ...
    "appendTextToNode(firstByClass(r,'bubble'),t);scrollBottom();}",
    ...
    "if(box)box.style.display='block';appendTextToNode(body,t);scrollBottom();}",
                                         ...
                                         const char *load_more_label)
{
  ...
  ok = ok &&
       ...
       strappy_webview_buffer_append_cstring(
         &buffer,
         "</div></div><script>renderMetadata(document);</script></body></html>");
