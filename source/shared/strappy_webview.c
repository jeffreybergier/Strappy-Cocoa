...
static int strappy_webview_append_styles(strappy_webview_buffer *buffer)
{
  static const char * const chunks[] = {
    ...
    ".api_function_output .role,.api_item .role{color:#30363b;}",
    ...
    ".api-exchange-metadata.api_error>.role{margin:0 -10px;",
    "padding:5px 10px;}",
...
#include "strappy_fontawesome_icons.inc"
    ...
    "setAPIExchangesCollapsedForPrompt(strappyProcessingPromptGroupKey,1);}",
    ...
    "function apiExchangeCollapsed(id,promptKey){if(promptKey!==''&&promptKey===strappyProcessingPromptGroupKey&&processingInteractionsLocked())return 0;",
    "return typeof strappyAPIExchangeCollapsed[id]=='undefined'?1:(strappyAPIExchangeCollapsed[id]?1:0);}",
    ...
    "for(i=0;i<keys.length;i++){g=groups[keys[i]];anchor=g.rows[0];",
    "collapsed=apiExchangeCollapsed(g.id,g.promptKey);previous='';last=g.rows[g.rows.length-1];",
    ...
    "function toolPromotedValue(name,args,dbNames,hintNames){var spec=toolDisplaySpec(name);var parsed=toolArgsValue(args);",
    "var key,value,transform,raw,parts,i;if(!spec||!isObj(parsed))return '';key=jsonText(spec.promoted_argument);",
    ...
    "function toolDisplayTitle(name,args,dbNames,hintNames){var title=jsonText(name||'Tool');",
    "var promoted=toolPromotedValue(name,args,dbNames,hintNames);return title+(promoted!==''?' \\u00b7 '+promoted:'');}",
                                   ...
                                   const char *status_html)
{
  ...
  ok = ok &&
       ...
       strappy_webview_append_data_attribute(
         &buffer,
         "tool-label",
         render_api_tool_card ? strappy_webview_tool_label(labels) : NULL) &&
       strappy_webview_append_data_attribute(&buffer,
                                             "tool-call-id",
                                             (message != NULL) ?
