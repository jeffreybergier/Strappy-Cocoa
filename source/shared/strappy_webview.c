...
#include "strappy_fontawesome_icons.inc"
    ...
    "setScrollTopPosition(scrollBottomPosition());}",
    ...
    "strappyScrollAnimationTimer=null;return;}now=(new Date()).getTime();",
    "progress=(now-started)/strappyScrollAnimationDuration;if(progress>=1){",
    "setScrollTopPosition(target);strappyScrollAnimationTimer=null;return;}",
    ...
    "if(!strappyAutoScrollEnabled)return;if(strappyBatchDepth>0){",
    ...
    "setScrollTopPosition(target);return;}started=(new Date()).getTime();",
    ...
    "function appendMessage(html){clearTimelineError();var m=byId('messages');var d,n,id,key,added=[];if(!m)return;",
    "if(!strappyMessageIndexesReady)initializeMessageIndexes();",
    "d=nodesFromHTML(html);while(d.firstChild){n=d.firstChild;id=rowId(n);key=rowMessageKey(n);",
    ...
    "function insertMessageBefore(id,html){clearTimelineError();var before=byId(id);var m=byId('messages');var n,key,rowIdentifier,added=[];",
    ...
    "if(!added.length)return;flushPendingToolTargets();renderAfterMutation(added);scrollBottomAnimated();}",
