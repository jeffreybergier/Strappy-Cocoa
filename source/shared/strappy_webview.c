...
#include "strappy_fontawesome_icons.inc"
    ...
    "function renderAPIServerToolRows(rows){rows=rows||apiServerToolRows();var i,row,kind,spec,item,title,error,bubble,body,summary,role,shown;",
    ...
    "body.innerHTML=toolPanel('Tool Call',name,toolInputHTML(name,raw,strappyDatabaseNames),raw,'');",
    "if(bubble)bubble.className=bubble.className.replace(/\\stool-error/g,'');}",
    "else{raw=apiToolRaw(row,'result-json');error=hasClass(row,'state-error')||toolOutputHasError(raw);body.innerHTML=toolOutputHTML(raw,name,error);",
