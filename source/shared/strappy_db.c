...
/* Canonical schema and query services. Connection policy is platform-owned. */
...
int strappy_db_ensure_schema(sqlite3 *db, char **error_out)
{
  ...
  if (strappy_db_connection_schema_is_ready(db)) {
    return 1;
  }
  ok = strappy_db_ensure_semantic_schema(db, error_out);
  if (ok) {
    strappy_db_connection_mark_schema_ready(db);
  }
  return ok;
}
