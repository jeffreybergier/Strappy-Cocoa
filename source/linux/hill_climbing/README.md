...
- web search and web fetch use the same selected OpenRouter engine;
...
Select one engine for both web tools with `WEB_PROVIDER=auto`,
`WEB_PROVIDER=native`, `WEB_PROVIDER=exa`, or `WEB_PROVIDER=parallel`
(the default is `none`):
